...
public class DatePickerPreference extends DialogPreference implements
    ...
    protected View onCreateDialogView() {
        ...
        int day = this.mSelectedDate.getDayOfMonth();
        int month = this.mSelectedDate.getMonthOfYear() - 1; // date picker
                                                             // months 0-11
        int year = this.mSelectedDate.getYear();
    ...
    public CharSequence getSummary() {
        String summary = "";
        if (isPersistent()) {
            long persistedDateInMillis = getPersistedLong(this.mSelectedDate.getMillis());
    ...
    protected void onSetInitialValue(boolean restorePersistedValue, Object defaultValue) {
        if (restorePersistedValue) // user has already set date, get persisted
                                   // value
        {
            // Restore existing state
            long persistedDate = this.getPersistedLong(this.mDefaultDate.getMillis());
            this.mSelectedDate = new DateTime(persistedDate);
        } else {
            // Set default state from the XML attribute
            long defaultDate = (Long) defaultValue;
            this.mSelectedDate = new DateTime(defaultDate);
    ...
    protected Object onGetDefaultValue(TypedArray a, int index) {
                                                ...
                                                // specified in the preferences
        {
            ...
            else // given date is specificed in the preferences
            {
                try // try to parse it
                {
                    this.mDefaultDate = DateTime.parse(defaultDateSpecifiedInPref,
                            DateUtils.SHORT_DATE_FORMAT);
    ...
    public void onDateChanged(DatePicker view, int year, int monthOfYear, int dayOfMonth) {
        ...
        this.mSelectedDate = new DateTime(year, monthOfYear, dayOfMonth, 0, 0);
    }
